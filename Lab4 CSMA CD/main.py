...
class MainWindow:
    def __init__(self):
        ...
        self.spinbox_collision_chance = Spinbox(from_=0, to=100, textvariable=self.spinbox_var, state='readonly',
                                                increment=10)
        self.spinbox_collision_chance.place(width=100, height=25, x=150, y=0)
        self.spinbox_var = StringVar(value='0')
        self.spinbox_busy_chance = Spinbox(from_=0, to=100, textvariable=self.spinbox_var, state='readonly',
                                           increment=10)
        self.spinbox_busy_chance.place(width=100, height=25, x=500, y=0)
        self.collision_chance_label = Label(self.root, text="Chance of collision", anchor="center", font="Arial 8")
        self.collision_chance_label.place(width=150, height=25, x=0, y=0)
        self.busy_chance_label = Label(self.root, text="Chance of busyness chanel", anchor="center", font="Arial 8")
        self.busy_chance_label.place(width=150, height=25, x=300, y=0)
    ...
    def send_button_click(self):
        ...
        if check_packet(packet):
            ...
            self.make_log(f"Sender's checksum = {checksum}")
            packet += checksum
            self.package_when_sending_textbox.config(state='normal')
            self.package_when_sending_textbox.insert(END, packet + '\n')
            self.package_when_sending_textbox.config(state='disabled')
            self.input_textbox.config(state='disabled')
            self.CSMA(packet)
            self.input_textbox.config(state='normal')
    ...
    def checkBusyChannel(self) -> bool:
        if random.randint(0, 99) < int(self.spinbox_busy_chance.get()):
            return True
        return False

    def checkCollision(self) -> bool:
        if random.randint(0, 99) < int(self.spinbox_collision_chance.get()):
            return True
        return False

    def CSMA(self, packet):
        global isCollisionHappened
        slot_time = 10
        for attempt in range(1, 11):
            countTry = 0
            while self.checkBusyChannel():
                countTry += 1
                self.make_log(f'Channel is busy!!![Failed: #{countTry}]')
                if countTry > 1000:
                    self.make_log(f'A lot of tries, channel permanently close]')
                    return
            self.send_data(packet)
            isCollisionHappened = self.checkCollision()
            if not isCollisionHappened:
                break
            self.make_log(f'Collision detect[#{attempt}]')
            k = min(attempt, 10)
            r = random.randint(0, int(2 ** k))
            var = IntVar()
            self.root.after(int(r * slot_time), var.set, 1)
            self.root.wait_variable(var)
        else:
            self.make_log(f"Can't send message!")
    ...
    def read_data(self):
        ...
        while 1:
            while self.port2.inWaiting() > 0:
                ...
                try:
                    out = self.port2.read_all()
                    out = out.decode('cp1251')
                except serial.PortNotOpenError:
                    self.make_log('Ports are closed')

            if out != '' and not isCollisionHappened:
                out_check_sum = crc16_generator(out[0:PACKET_LENGTH_DEC_STRING])
                if out_check_sum != out[PACKET_LENGTH_DEC_STRING:]:
                    self.make_log(f"Transmission error: sender and receiver checksums are not equal. Recipient "
                                  f"checksum = {out_check_sum}")
                else:
                    self.make_log(f"Recipient checksum = {out_check_sum}")
